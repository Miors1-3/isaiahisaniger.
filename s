...
local Window = Fluent:CreateWindow({
    ...
    Acrylic = false, --ALWAYS FALSE, MIOR U NIGGER
    Theme = "Dark",
...
local Tabs = {
    ...
    AutoFarm = Window:AddTab({ Title = "Auto Farm", Icon = "hand-coins" }),
...
Section6:AddButton({
    ...
    Callback = function()
        ...
        local function teleportTo(position)
            ...
            plr.Character.HumanoidRootPart.CFrame = CFrame.new(position.Position.X + 2, position.Position.Y, position.Position.Z)
        end
...
local ATSection = Tabs.AutoFarm:AddSection("AutoFarm")


ATSection:AddToggle("AutoRobStudio", { Title = "Auto Rob Studio", Default = false }):OnChanged(function()
    local robstudio = Options.AutoRobStudio.Value
    ...
    if robstudio then
		...
		for _, v in pairs(workspace.StudioPay.Money:GetDescendants()) do
			if v:IsA("ProximityPrompt") and v.Name == "Prompt" and v.Enabled then
				teleportToLocation(CFrame.new(v.Parent.Position.X, v.Parent.Position.Y + 2, v.Parent.Position.Z))
		...
		teleportToLocation(OldCFrameStudio)
	end
end)
--[[
